...
// Fallback only. Callers should pass the origin of the site the ambassador is
// actually on (window.location.origin on the client, the request's origin on
// the server): the custom domain has been unreachable before, and a link built
// from a hard-coded host then sends every referred student to a dead page.
export const REFERRAL_LINK_BASE = "https://trybemarket.online";

export function buildReferralLink(code, origin = REFERRAL_LINK_BASE) {
