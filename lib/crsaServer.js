 ...
 *   crsaStats/{uid}/referrals/{referredUid} { countedAt }   — fixed-ID idempotency marker (KYC)
 *   crsaStats/{uid}/installs/{referredUid}  { countedAt }   — fixed-ID idempotency marker (app install)
 *   referralAttachFailures/{uid}            { code, reason, at } — why a supplied ref code did NOT attach
...
export async function prepareReferralKycCount(tx, userId) {
  const referrer = await readReferrer(tx, userId);
  if (!referrer) return null;
  const { referrerUid, referredBy, statsRef } = referrer;

  const memberRef = adminDB.collection("crsaMembers").doc(referrerUid);
  const markerRef = statsRef.collection("referrals").doc(userId);
  const installMarkerRef = statsRef.collection("installs").doc(userId);
  const pwaRef = adminDB.collection("pwaInstalls").doc(userId);
  const [memberSnap, statsSnap, markerSnap, installMarkerSnap, pwaSnap] = await Promise.all([
    tx.get(memberRef),
    tx.get(statsRef),
    tx.get(markerRef),
    tx.get(installMarkerRef),
    tx.get(pwaRef),
  ]);
  ...
  return {
    ...
    // An install reported while this user was still unverified was recorded but
    // held (see prepareInstallCount): it becomes countable the moment they
    // verify, so it is counted in this same transaction.
    installPending: pwaSnap.exists && !installMarkerSnap.exists,
    installMarkerRef,
    pwaRef,
  };
}
...
export function applyReferralKycCount(tx, plan) {
  ...
  tx.set(plan.markerRef, { countedAt: now, referralCode: plan.referredBy });
  if (plan.installPending) {
    tx.set(plan.installMarkerRef, { countedAt: now, referralCode: plan.referredBy, via: "kyc" });
    tx.update(plan.pwaRef, { countedForCrsa: true });
  }
  if (plan.statsExists) {
    tx.update(plan.statsRef, {
      kycCompletions: admin.firestore.FieldValue.increment(1),
      ...(plan.installPending ? { installs: admin.firestore.FieldValue.increment(1) } : {}),
      updatedAt: now,
    });
  } else {
    ...
    tx.set(plan.statsRef, {
      kycCompletions: 1,
      installs: plan.installPending ? 1 : 0,
      updatedAt: now,
    });
  }
}

// --- Counting installs (the secondary stat) --------------------------------

/**
 * An app install counts for a referrer only if the installing user is a
 * VERIFIED referral — i.e. their KYC referral marker exists. Installs can't be
 * verified server-side, so tying them to a KYC-verified referral is what keeps
 * fake referred accounts from inflating the number. Two consequences:
 *   - installs are always <= kycCompletions for a given ambassador (the UI
 *     shows "N of M verified referrals" so a low number reads as expected);
 *   - an install reported before KYC is held and counted inside the KYC
 *     transaction (prepare/applyReferralKycCount above), not lost.
 * The install marker doc is the idempotency key, as with the KYC marker.
 *
 * Phase 1, READS ONLY (same prepare/apply split as above). Returns null when
 * nothing should be counted.
 */
export async function prepareInstallCount(tx, userId) {
  const referrer = await readReferrer(tx, userId);
  if (!referrer) return null;
  const { referrerUid, referredBy, statsRef } = referrer;

  const memberRef = adminDB.collection("crsaMembers").doc(referrerUid);
  const kycMarkerRef = statsRef.collection("referrals").doc(userId);
  const installMarkerRef = statsRef.collection("installs").doc(userId);
  const [memberSnap, statsSnap, kycMarkerSnap, installMarkerSnap] = await Promise.all([
    tx.get(memberRef),
    tx.get(statsRef),
    tx.get(kycMarkerRef),
    tx.get(installMarkerRef),
  ...
  if (!memberSnap.exists || !memberSnap.data().active) return null;
  if (!kycMarkerSnap.exists) return null; // not a verified referral (yet)
  if (installMarkerSnap.exists) return null; // already counted

  return { referredBy, statsRef, installMarkerRef, statsExists: statsSnap.exists };
}

/** Phase 2 — WRITES ONLY, given a non-null plan from prepareInstallCount. */
export function applyInstallCount(tx, plan) {
  const now = admin.firestore.FieldValue.serverTimestamp();
  tx.set(plan.installMarkerRef, { countedAt: now, referralCode: plan.referredBy, via: "install" });
  if (plan.statsExists) {
    tx.update(plan.statsRef, { installs: admin.firestore.FieldValue.increment(1), updatedAt: now });
  } else {
    tx.set(plan.statsRef, { kycCompletions: 0, installs: 1, updatedAt: now });
  }
}

/**
 * Resolves user -> referral code -> referrer uid. READS ONLY. Null if the user
 * has no referral, the code is unknown, or it's a self-referral (also blocked
 * at attach time — defense in depth).
 */
async function readReferrer(tx, userId) {
  ...
  return { referrerUid, referredBy, statsRef: adminDB.collection("crsaStats").doc(referrerUid) };
}

// --- Attribution failures ---------------------------------------------------

/**
 * Persists why a referral code the client supplied did NOT attach (unknown
 * code, deactivated member, account too old, …). Without it the reason lived
 * only in a console.log, so an ambassador whose numbers looked low had no
 * trail to follow. Server-only collection; best-effort — never throws. Only
 * failures the server can see: a code lost on the client before signup never
 * reaches here.
 */
export async function recordReferralAttachOutcome(uid, rawCode, result) {
  try {
    const ref = adminDB.collection("referralAttachFailures").doc(uid);
    if (result.attached) {
      await ref.delete();
    } else if (result.reason !== "already_attributed") {
      // already_attributed is the harmless repeat of a call that already succeeded.
      await ref.set({
        code: String(rawCode).slice(0, 32),
        reason: result.reason,
        at: admin.firestore.FieldValue.serverTimestamp(),
      });
    }
  } catch (error) {
    console.error("Error recording referral attach outcome:", error);
  }
}
 ...
 *    as the members table on the CRSA page of the separate trybemarket-admin app.
