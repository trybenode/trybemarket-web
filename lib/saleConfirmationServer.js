...
export async function confirmSale(conversationId, uid) {
  ...
  const result = await adminDB.runTransaction(async (tx) => {
    ...
    tx.set(
      conversationRef,
      { buyerId, sellerId, saleStatus: bothConfirmed ? "confirmed" : "pending", buyerConfirmedAt, sellerConfirmedAt },
      { merge: true }
    );
...
export async function markSalePending(conversationId, uid) {
  ...
  return adminDB.runTransaction(async (tx) => {
    ...
    if (!conv.saleStatus || conv.saleStatus === "none") {
      tx.set(conversationRef, { buyerId, sellerId, saleStatus: "pending" }, { merge: true });
