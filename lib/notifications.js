...
// Daily cap for shop-view WhatsApp alerts, kept separate from the message-notification
// quota above. Same value for Premium and VIP for now — change here to adjust for everyone.
export const SHOP_VIEW_DAILY_NOTIFICATION_LIMIT = 2;
...
/**
 * SHOP-VIEW NOTIFICATION QUOTA - CHECK ONLY (does not increment)
 * Independent of the message-notification WhatsApp quota above.
 * @param {string} sellerId - User ID of the shop owner
 * @returns {Promise<{allowed: boolean, remaining: number, limit: number, currentCount: number}>}
 */
export async function checkShopViewNotificationQuota(sellerId) {
  try {
    const userRef = adminDB.collection("users").doc(sellerId);
    const userSnap = await userRef.get();

    if (!userSnap.exists) {
      return { allowed: false, remaining: 0, limit: SHOP_VIEW_DAILY_NOTIFICATION_LIMIT, currentCount: 0 };
    }

    const userData = userSnap.data();
    const today = getTodayString();
    const limit = SHOP_VIEW_DAILY_NOTIFICATION_LIMIT;

    const shopViewData = userData.notifications?.shopViewNotified || { dailyCount: 0, lastResetDate: "" };
    let currentCount = shopViewData.dailyCount || 0;

    if (shopViewData.lastResetDate !== today) {
      currentCount = 0;
    }
    ...
    console.error("Error checking shop-view notification quota:", error);
    return { allowed: false, remaining: 0, limit: SHOP_VIEW_DAILY_NOTIFICATION_LIMIT, currentCount: 0 };
  }
}

/**
 * SHOP-VIEW NOTIFICATION QUOTA - INCREMENT (call AFTER successful send)
 * @param {string} sellerId - User ID of the shop owner
 * @returns {Promise<{success: boolean, remaining: number, limit: number}>}
 */
export async function incrementShopViewNotified(sellerId) {
  try {
    const userRef = adminDB.collection("users").doc(sellerId);
    const userSnap = await userRef.get();
    const limit = SHOP_VIEW_DAILY_NOTIFICATION_LIMIT;
    ...
    if (shopViewData.lastResetDate !== today) {
      currentCount = 0;
      await userRef.update({
        "notifications.shopViewNotified.dailyCount": 0,
        "notifications.shopViewNotified.lastResetDate": today,
      });
    }

    await userRef.update({
      "notifications.shopViewNotified.dailyCount": FieldValue.increment(1),
      "notifications.shopViewNotified.lastResetDate": today,
    });

    return { success: true, remaining: limit - currentCount - 1, limit };
  } catch (error) {
    console.error("Error incrementing shop-view notification count:", error);
    return { success: false, remaining: 0, limit: SHOP_VIEW_DAILY_NOTIFICATION_LIMIT };
  }
}
