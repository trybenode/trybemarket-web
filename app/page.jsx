  ...
  const fetchProducts = async (isLoadMore = false, isRefresh = false) => {
    try {
      ...
      if (isLoadMore && lastVisible) {
        q = query(baseQuery, orderBy("createdAt", "desc"), startAfter(lastVisible), limit(PAGE_SIZE))
      }
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      <div className="flex items-center justify-between p-4">
        <Image
          src="/logo.jpeg?height=50&width=150"
          alt="Logo"
          width={150}
          height={50}
      ...
      <div className="flex-1 px-3">
        ...
        {productsToDisplay.length > 0 || loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-[50vh]">
            <button onClick={onRefresh} className="mb-4 text-blue-500 hover:underline">
              Refresh
            </button>
            <p className="mx-4 text-center text-lg text-red-500">
              {hasSearchQuery ? "No products found" : "Please check your internet connection and refresh"}
            </p>
          </div>
        )}
      </div>
